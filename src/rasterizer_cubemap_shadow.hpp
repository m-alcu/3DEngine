...
// Specialized rasterizer for cubemap shadows
template<class Effect>
class CubemapShadowRasterizer {
public:
    ...
    void drawRenderable(TransformComponent& transform,
        ...
        shadowComponent = shadowSrc;
        currentFace = face;
        screenWidth = shadowComponent->shadowMap->cubeShadowMap->faceSize;
        screenHeight = shadowComponent->shadowMap->cubeShadowMap->faceSize;

        // Set the current face for the effect
        effect.setFace(face);
    ...
    void rasterizeFilledPolygon(Polygon<vertex>& polygon) {
        ...
        walker.walk([&](int xStart, int xEnd, int dx, Slope<vertex>& left, Slope<vertex>& right) {
            ...
            for (int x = xStart; x < xEnd; ++x) {
                effect.ps(x, p_z, *shadowComponent->shadowMap->cubeShadowMap);
                p_z += p_z_step;
            }
        });
    }
