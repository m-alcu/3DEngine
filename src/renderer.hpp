...
private:
  ...
  void drawAxes(Scene &scene, float axisLength = 1000.0f) {
    const slib::vec3 origin{0.0f, 0.0f, 0.0f};
    drawAxisLine(scene, origin, {axisLength, 0.0f, 0.0f}, 0xffff0000);
    drawAxisLine(scene, origin, {0.0f, axisLength, 0.0f}, 0xff00ff00);
    drawAxisLine(scene, origin, {0.0f, 0.0f, axisLength}, 0xff0000ff);
    drawGridPlanes(scene);
  }

  void drawGridPlanes(Scene &scene, float halfSize = 500.0f,
                      float spacing = 50.0f) {
  ...
  void drawAxisLine(Scene &scene, const slib::vec3 &start,
  ...
  bool clipLineNdc(AxisVertex &a, AxisVertex &b) {
  ...
  void drawLineWithDepth(Scene &scene, const AxisVertex &v0,
