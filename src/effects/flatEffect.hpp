...
public:
  ...
  public:
    ...
    Fragment(float pz) : p_z(pz) {}

    Fragment operator+(const Fragment &f) const {
      return Fragment(p_z + f.p_z);
    }
    ...
    Fragment operator*(const float &rhs) const { return Fragment(p_z * rhs); }
  ...
  public:
    static Fragment fromVertex(const Vertex &v) { return Fragment(v.p_z); }
