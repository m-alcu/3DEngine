...
public:
  ...
  public:
    ...
    Fragment(float pz, float diff) : p_z(pz), diffuse(diff) {}

    Fragment operator+(const Fragment &f) const {
      return Fragment(p_z + f.p_z, diffuse + f.diffuse);
    }
    ...
    Fragment operator*(const float &rhs) const {
      return Fragment(p_z * rhs, diffuse * rhs);
    }
  ...
  public:
    static Fragment fromVertex(const Vertex &v) {
      return Fragment(v.p_z, v.diffuse);
    }
  ...
  public:
    void operator()(Polygon<Vertex> &poly, const Scene &scene) const {
      ...
      for (auto &point : poly.points) {
        ...
        point.diffuse = std::max(
            0.0f, smath::dot(point.normal, luxDirection)); // diffuse scalar
  ...
  public:
    uint32_t operator()(Fragment &frag, const Scene &scene,
      ...
      return Color(poly.material.Ka + poly.material.Kd * frag.diffuse).toBgra();
    }
  };
