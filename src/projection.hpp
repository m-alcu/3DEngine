...
public:  
   ...
   void view(const int32_t width, const int32_t height, vertex& p, bool init) {
       ...
       if (p.broken || init) {
           ...
           float sx = (p.ndc.x * oneOverW * 0.5f + 0.5f) * width + 0.5f; // Convert from NDC to screen coordinates
           float sy = (p.ndc.y * oneOverW * 0.5f + 0.5f) * height + 0.5f; // Convert from NDC to screen coordinates
