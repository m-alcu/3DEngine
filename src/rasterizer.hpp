...
template <class Effect> class Rasterizer {
public:
  using vertex = typename Effect::Vertex;
  using fragment = typename Effect::Fragment;
...
#pragma omp parallel for
    for (int i = 0; i < (int)solid->vertexData.size(); ++i) {
      projectedPoints[i] = effect.vs(solid->vertexData[i], fullTransformMat,
                                     normalTransformMat, *scene);
    }
  }
...
#pragma omp parallel for
    for (int i = 0; i < static_cast<int>(solid->faceData.size()); ++i) {
      ...
      rotatedFaceNormal =
          normalTransformMat * slib::vec4(faceDataEntry.faceNormal, 0);
      ...
      if (solid->shading == Shading::Wireframe ||
        ...
        Polygon<vertex> poly(
            std::move(polyVerts), faceDataEntry.face, rotatedFaceNormal,
  ...
  This is a simplified version of the backface culling algorithm.
          If the dot product of the face normal and the view direction is
  positive, the face is visible.
  */
  ...
  The algorithm works by iterating through each scanline of the polygon and
  determining the left and right edges at that scanline. For each scanline, the
  algorithm calculates the x-coordinates of the left and right edges and fills
  in the pixels between them. The algorithm uses a slope to determine the
  x-coordinates of the left and right edges of the triangle at each scanline.
  */
  ...
  void drawPolygon(Polygon<vertex> &polygon) {
    ...
    // Find the point that is topleft-most. Begin both slopes (left & right)
    // from there. Also find the bottomright-most vertex; that’s where the
    // rendering ends.
    auto cmp_top_left = [&](const vertex &a, const vertex &b) {
      return std::tie(a.p_y, a.p_x) <
             std::tie(b.p_y, b.p_x); // top-left is "smaller"
    };
    ...
    for (int side = 0, cury = gety(side), nexty[2] = {cury, cury},
             hy = cury * scene->screen.width;
         cur[side] != last;) {
      // We have reached a bend on either side (or both). "side" indicates which
      // side the next bend is. In the beginning of the loop, both sides have a
      // bend (top-left corner of the polygon). In that situation, we first
      // process the left side, then without rendering anything, process the
      // right side. Now check whether to go forwards or backwards in the
      // circular chain of points.
      ...
      // Identify which side the next bend is going to be, by choosing the
      // smaller Y coordinate.
      side = (nexty[0] <= nexty[1]) ? 0 : 1;
      // Process scanlines until the next bend.
      for (int limit = nexty[side]; cury < limit;
           ++cury, hy += scene->screen.width)
        drawScanline(hy, slopes[0], slopes[1], polygon, pixels);
    }
  };

  // Convert Vertex to Fragment for rasterization
  inline fragment toFragment(const vertex &v) const {
    return fragment::fromVertex(v);
  }

  inline void drawScanline(const int &hy, Slope<vertex> &left,
                           Slope<vertex> &right, Polygon<vertex> &polygon,
                           uint32_t *pixels) {
    ...
    if (dx != 0) {
      float invDx = 1.0f / dx;
      vertex vLeft = left.get();
      vertex vRight = right.get();

      // Convert to fragment at scanline edges
      fragment fStart = toFragment(vLeft);
      fragment fStep = (toFragment(vRight) - fStart) * invDx;

      for (int x = xStart; x < xEnd; ++x) {
        int index = hy + x;
        if (scene->zBuffer->TestAndSet(index, fStart.p_z)) {
          pixels[index] = effect.ps(fStart, *scene, polygon);
        }
        fStart += fStep;
      }
    }
  ...
  void drawWireframePolygon(Polygon<vertex> polygon, uint32_t color,
    ...
    for (size_t i = 0; i < polygon.points.size(); i++) {
      ...
      auto &v1 =
          polygon.points[(i + 1) % polygon.points.size()]; // wrap back to first

      drawBresenhamLine(v0.p_x >> 16, v0.p_y >> 16, v1.p_x >> 16, v1.p_y >> 16,
                        pixels, width, height, color);
    }
  }

  void drawBresenhamLine(int x0, int y0, int x1, int y1, uint32_t *pixels,
